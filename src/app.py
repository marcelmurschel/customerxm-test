...
app.layout = html.Div([
    ...
    html.Div([
        ...
        html.Div([
            html.H2('Anzahl Interviews'),
            html.P(id='respondent-count', style={'fontSize': '24px'})
    ...
    html.Div([
        html.Label('Adjust Threshold Percentage:'),
        dcc.Slider(
            id='threshold-slider',
            min=0,
            max=30,
    ...
    html.Div([
        html.Label('Adjust Rating Threshold:'),
        dcc.Slider(
    ...
    ], style={'width': '24%', 'display': 'inline-block', 'verticalAlign': 'top', 'padding': '0 10px'}),

    html.Div([
        html.Label('Standort of Interest:'),
        dcc.Dropdown(
...
], style={'marginTop': '75px', 'marginBottom': '20px', 'display': 'flex'}),
...
], style={'maxWidth': '1400px', 'margin': '0 auto', 'padding': '20px', 'border': '1px solid #ccc', 'boxShadow': '2px 2px 12px #aaa'})
...
def update_dashboard(selected_standorte, threshold, rating_threshold, selected_topic, selected_standort, start_date, end_date, review_rating):
    ...
    'data': traces_line,
    'layout': go.Layout(
        title='Durchschnittliche Zufriedenheit pro Quartal',
    ...
    for topic in topics:
        ...
        row['Total'] = round(total_avg_rating, 2) if not np.isnan(total_avg_rating) else 'N/A'
        for standort in selected_standorte:
            filtered_reviews = filtered_data[(filtered_data['name'] == standort) & (filtered_data[topic] == 1)]
            avg_rating = filtered_reviews['Rating'].mean()
            row[standort] = round(avg_rating, 2) if not np.isnan(avg_rating) else 'N/A'
